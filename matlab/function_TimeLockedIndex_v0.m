...
%          'plot'    -> Flag to plot the signals (logical).
%                       false => Do not plot.
%                       true  => Plot the signals involved in the TLI computation.
...
if ~isfield(TLIcfg, 'fs') || isempty(TLIcfg.fs) || ~isnumeric(TLIcfg.fs) || ~isfinite(TLIcfg.fs),
    TLIcfg.fs = 1; %Default value.
end

if ~isfield(TLIcfg, 'NT') || isempty(TLIcfg.NT) || ~isnumeric(TLIcfg.NT) || ~isfinite(TLIcfg.NT), 
    TLIcfg.NT = 1; %Default value.
end
...
HFSignaltimeLockedLFpeaks = [];
TLI = [];
%--------------------------------------------------------------------------

%Parameters ---------------------------------------------------------------
%Compute the number of samples for the lower cutoff frequency.
lowestFreq = TLIcfg.BPFcfg_LF.f0 - TLIcfg.BPFcfg_LF.Bw/2;
sampleT = round( TLIcfg.NT * TLIcfg.fs / lowestFreq );
...
%resulting TLI value is constrained to the interval [0,1].
