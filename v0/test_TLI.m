...
%Authors: Osvaldo Velarde, Damian Dellavale.
...
FREQ_BANDS = [1, 15;...   %[Hz] Low-frequency band.
              20, 100;... %[Hz] High-frequency band.
              ];    
...
for ii=1:+1:length(mu), %Loop across the mu values.
    ...
    meanSignal = mean(rawSignal,1);

    %Include additive white Gaussian noise.
    noise = (NOISE_LEVEL * stdDevSignal) * rand(size(rawSignal)) + meanSignal;
